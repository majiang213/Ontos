...
| 候选对列表 | `GET /api/list_candidates` | 允许只读（`list_candidates`） | Agent 看清单后请人点「待确认」定案；不定案、不算交集率 |
...
- **`list_candidates` 已落地：** 只读开放（`mcp/tools.ts`），Agent 看清单后请人点「待确认」定案。`overlap`（算交集率）仍无 MCP：关卡在人，不经 Agent。
