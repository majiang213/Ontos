...
**MVP 演示链路（30 分钟）**：连采购、设备、资产三个老库 → AI 生成对象草稿 → 人裁决两对候选对：在途设备 × 在役设备（序列号交集率约三分之一）裁为**生命周期**，设备 × 资产台账裁为**类等价** → 发布本体 → **问数**「在役设备及其所属部门」（附取数路径）→ **验收一台在途设备**：设备源、资产源各插入一行，并立一张保修卡 → 再问，这台设备的阶段已是在役。
