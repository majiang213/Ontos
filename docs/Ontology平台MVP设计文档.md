...
> V2.4 变更：裁决分级——合并类永远人定案，同形异义（旧称仅名称相似）类可升级为机器定案、人抽检，升级节奏由裁决接受率决定（§2、§3 M3）。
...
**MVP 演示链路（30 分钟）**：连采购、设备、资产三个现有数据库 → AI 生成对象草稿 → 人裁决两对候选对：在途设备 × 在役设备（序列号交集率约三分之一）裁为**生命周期**，设备 × 资产台账裁为**类等价** → 发布本体 → **问数**「在役设备及其所属部门」（附取数路径）→ **验收一台在途设备**：设备源、资产源各插入一行，并立一张保修卡 → 再问，这台设备的阶段已是在役。
...
**合并决策不是二元判断，而是对齐判定（不是类与类关系的穷尽）。** 人判定的是这一对候选类描述的现实：是不是同一种事物、是不是同一批个体、是不是同一批个体的不同时期。名字和表结构都不是依据。

| 对齐判定 | 情形 | 写进本体 |
|---|---|---|
| 类等价 | 两个类描述同一种现实事物 | 合并为一个对象类型，挂多个源 |
| 部分重叠 | 两个类描述的个体有交集、又不是同一批 | 另立上位对象。同名公共属性移上去；没有同名则上位对象只带同一性标准。特有属性留在原类。写入 `class_conclusions` |
...
**裁决按后果分级，「裁决权在人」的行使方式不同。** 合并类（类等价、部分重叠、生命周期）改写本体结构，判错比不合并更糟，这类永远由人定案。注意交集率 100% 也可能是假证据：两端的识别字段若都是自增 ID，值域天然重合，「证据硬」不等于「可以自动并」。同形异义写入 `class_conclusions`。跳过不改配置，定错了以后能翻回来，这类可以升级自动化。自动化的形态是预分拣，不是替人裁：机器把高置信的「各自独立」直接定案，人批量确认、事后抽检；合并类候选永远排进裁决队列等人定案。人从逐题作答变成保留否决权——库大、候选对多时，这是唯一扛得住的形态。
...
| M2 AI 逆向建模 | 表结构 → 本体草稿 | `propose_objects` 只建议；`import_objects` 才上画布。人改字段，或再勾表生成。无 ReAct，没有「再说一句」出口 |
| M3 整合工作台 | 候选对、交集验证、结论裁决（子类型本期不定案；同形异义类可由机器定案、人抽检）、留痕 | **差异化核心** |
...
切片只示 `convert` 一条动作；调拨、报废、登记、结束维修与保修卡类见《ontos-article.md》附录 C。对齐判定在配置里的表达：类等价 = 一个类挂多个 `sources`；部分重叠 = 上位对象（同名属性上移；没有同名则只带同一性标准）并写入 `class_conclusions`；生命周期 = 派生属性加 `transition` 关系加转化动作；同形异义 = 写入 `class_conclusions`，不建个体边、源映射不共用；子类型 = 本期不定案。跳过不写配置。
...
**Demo 脚本**（mock 采购 + 设备 + 资产三个库）：配置连接 → 生成对象（设备库同时带出部门对象和「属于」关系）→ 两对候选对分别裁为生命周期、类等价 → 发布本体 → 问「在役设备及其所属部门」（展示取数路径）→ 验收一台在途设备（设备源、资产源各插入一行，立一张保修卡）→ 再问，这台设备已在役。全程 ≤30 分钟含讲解，没有「一键生成新系统」。
...
**生成要模型，执行不要。** 模型只产草稿（对象、关系、动作定义），人裁决、发布。执行器解释已发布配置，禁自由 SQL。画布上的再生成是人驱动的：再勾表点「生成对象」——`propose_objects` 一次、`import_objects` 落地，停。没有「再说一句」出口。模型不自己选工具、不自己写库。
...
| 同形异义 | 两个类不是在描述同一种东西 | 已落地。写入 `class_conclusions`，不建个体边、源映射不共用 |
