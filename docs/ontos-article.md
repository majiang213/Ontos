...
现有系统要用上本体，第一步是从系统逆向建模：读它的表结构和数据，认出类、属性和关系。系统一多，多一层问题：每个系统只存现实的一个侧面，各起各的名字，各建各的表——这个系统的某个类和那个系统的某个类是什么关系，必须先判定；同一库里的两张表也可能指的是同一种东西。这一步就是本体对齐。对齐的结论有五种。定义是各结论是什么；判定办法是对这一对候选类问三问，用各自的证据，再合成一种结论。
...
| 结论 | 情形 | 处理 |
|---|---|---|
| 同一 | 两个类描述同一种现实事物 | 合并为一个**对象类型**，挂多个源 |
| 部分重叠 | 两个类描述的个体有交集、又不是同一批 | 另立**上位对象**。同名公共属性移上去；没有同名则上位对象只带同一性标准。特有属性留在原类 |
| 阶段 | 两个类描述的是同一个体的不同时期 | 立一个**对象类型**；阶段是**派生属性**，读时按规则现算；**转化关系**由**动作**写入时记录 |
| 仅名称相似 | 两个类不是在描述同一种东西 | 各自独立，互不映射 |
| 子类型 | 一类完全含于另一类 | 声明**子类型**：个体自动是上位类的个体，继承它的全部属性 |

注意部分重叠和子类型的区别：上位对象不带继承语义，属性是移上去的，不是继承来的。五种结论的判定都不靠名字、不靠表结构是否相似。

#### 判定办法

被判定的是已上画布、尚未定案的两个类。可以来自不同库，也可以是同一库里的两张表。结构是否相似不是入围资格。结构比对列出可能有关的对：名称相近或字段能对上的，容易被送进候选；表是多行对多列、字段完全对不上的同一，这一步会漏掉。同名不同种的对，这一步会误送进来。没挂来源的对象不进判定。

这些对把类牵成**疑似重复串**：A 与 B 是一对、B 与 C 是一对，则 A、B、C 是一串。一次仍只判定一对。裁「同一」或「阶段」之后，被并掉的类所牵着的类，改问留下的类——即使当初没有列出过这一对。裁「部分重叠」之后，原来跟这两类牵着的对按现在的类再问。裁「仅名称相似」或「跳过」只拿掉这一对。上一对的结论不代替下一对的三问。没被任何对牵上的类不进串。人不指定两个对象来比（本期未做），也不靠扫表里的行来补漏。

同一性标准是第二问的前提，不是同一的前提。没设同一性标准，算不了交集率，人仍可按第一问、第三问定案。
...
**第二问：是不是同一批个体。** 证据是交集率。取值是该类在该源上、按同一性标准取出的个体标识，归一化后做集合重合。个体在源里怎么落，不改变同一的定义：一行一个体，取同一性标准对照的那一列；一列一个体，取列所标识的那个个体。本期只落地行存。没设同一性标准，或有一侧取不出任何取值，这一问沉默。
...
| 否 | （任意） | （任意） | 仅名称相似 |
| 是 | 命中大于零 | 是 | 阶段 |
| 是 | 命中大于零，又不是全交 | 否 | 同一或部分重叠：并成一个类，或保留两个类并立公共对象。人选落地 |
| 是 | 全交，或沉默，或命中为零 | 否 | 同一 |
| （这次不定） |  |  | 跳过 |

两条硬约束：命中大于零时，数据不支持仅名称相似。命中为零时，数据不支持部分重叠（部分重叠的定义要求个体有交集）。人仍可否决数据，那是不顾证据。

采购「在途设备」和设备「在役设备」：第一问是同一种事物（设备）；第二问交集约三分之一，同一批里只有一部分现在两边都有；第三问人定为不同时期。结论是阶段。两个都叫「账户」、一个资金一个登录：第一问不是同一种事物，结论是仅名称相似，不必等交集率。资产表还没有行、设备表有行：第二问沉默，不能据此否定同一。
...
图 4：人裁的是第 2 节的类间关系与生命周期，定案后改本体。类等价：两个类描述同一种事物，写成一个类，两边的表都写成源映射。部分重叠：另立上位对象。同名公共属性移上去、不带继承语义；没有同名则上位对象只带同一性标准。特有属性留在原类。生命周期：收成一个类，阶段用派生属性读时现算，转化写成关系，由动作写入。同形异义：两个类都留下，不建关系，源映射也不共用。子类型：一类完全含于另一类，只声明继承、不抽公共属性，本期不做。
...
经过上面两节，配置中已有**类**、**同一性标准**和**源映射**。对齐判定的是这两个被比对的**类**之间的**关系**，该项为**类等价**、**部分重叠**、**生命周期**、**同形异义**或**子类型**之一。
...
| 本体对齐 | 判定两个类是什么关系（不同库，或同一库的两张表）；结论五种：同一、部分重叠、阶段、仅名称相似、子类型 |
