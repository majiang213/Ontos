...
本系统把上述规定写成一份可发布的模型，并用它访问已有系统。多个源上的对象由 Agent 按证据归纳：是不是同一个概念、该合并还是建一条链，然后进入同一份本体。查询按类型与关系读原库。写入先说明存在上发生了什么（阶段变化、新生一个关联对象等），再写回相应的源。换领域只改这份模型。
...
**个体之间的边是关系。** 关系把一类的个体连到另一类的个体上：设备「归属于」部门，每台设备连一个部门，一个部门可以连多台设备——从谁到谁是方向，允许连几个是基数。关系的取值是另一个体，这是它和属性的分界：属性的取值是字面值。

**类与类。** 两个类还可以是类等价、部分重叠、子类型、同形异义、部分与整体、不相交。这不是封闭清单。对齐候选对时只问其中若干项。类等价是两个类描述同一种现实事物，因而当作一个类；不是因为名字相同。子类型是一类完全落在另一类里面：它的每个个体同时是上位类的个体，并继承上位类的全部属性——上位类就是这条关系的另一端，不是另一种关系。部分重叠是两类的个体有交集、又各有一部分不属于对方。判定看个体在同一性标准下的重合，不看属性是否同名。处理它不用子类型：落地时并入同一个多源类（一个对象挂全部来源），不抽公共部分另立上位对象。同形异义是两个类不是在描述同一种东西：两个系统里都有「账户」，一个指资金账户，一个指登录账户，就不能并成一类。部分与整体是组成关系：发动机是车的一部分。它和子类型常被混淆，判别方法很简单：子类型的每个个体同时是上位类的个体，而一台发动机不是一台车，所以它是部分，不是子类型。
...
现有系统要用上本体，第一步是从系统逆向建模：读它的表结构和数据，归纳出概念（对象）、属性和关系——几张表在描述同一个概念的，归并成一个对象。系统一多，多一层问题：每个系统只存现实的一个侧面，各起各的名字，各建各的表——这个系统的某个类和那个系统的某个类是什么关系，必须先判定；同一库里的两张表也可能指的是同一种东西。这一步就是本体对齐。对齐时对候选对的判定是封闭的几项，不是类与类关系的穷尽。定义是各项是什么；判定办法是对这一对候选类问三问，用各自的证据，再合成一种结论。

#### 对齐时的判定

判定看这一对候选类描述的现实：是不是同一种事物、是不是同一批个体、是不是同一批个体的不同时期。名字、表是多行还是多列、字段是否同名，都不是依据。

| 结论 | 情形 | 处理 |
|---|---|---|
| 类等价 | 两个类描述同一种现实事物 | 合并为一个**对象类型**，挂多个源 |
| 部分重叠 | 两个类描述的个体有交集、又不是同一批 | 并入同一个多源类——一个对象挂全部来源（ADR 0013）。结论与证据住裁决工作记录（`adj_decision`），不进配置 |
| 生命周期 | 两个类描述的是同一个体的不同时期 | 立一个**对象类型**；阶段是**派生属性**，读时按规则现算；**转化关系**由**动作**写入时记录 |
| 同形异义 | 两个类不是在描述同一种东西 | 两类都留下，配置不动；结论与证据住裁决工作记录（`adj_decision`）。不建个体边、源映射也不共用 |
| 子类型 | 一类完全含于另一类 | 声明**子类型**：个体自动是上位类的个体，继承它的全部属性。本期不定案 |
| 部分与整体 | 一个是另一个的组成部分；部分的个体不是整体的个体 | 本期不定案（概念见第 2 节） |
| 不相交 | 两类没有共同的个体 | 本期不定案 |

注意部分重叠和子类型的区别：部分重叠的两类并入同一个多源类，互不继承。对齐时的判定都不靠名字、不靠表结构是否相似。标「本期不定案」的结论对齐时不判，配置里也没有对应构造；它们的概念定义见第 2 节。

#### 判定办法

被判定的是已上画布、尚未定案的两个类。可以来自不同库，也可以是同一库里的两张表。结构是否相似不是入围资格。结构比对列出可能有关的对：名称相近或字段能对上的，容易被送进候选；表是多行对多列、字段完全对不上的类等价，这一步会漏掉。同名不同种的对，这一步会误送进来。没挂来源的对象不进判定。

这些对把类牵成**疑似重复串**：A 与 B 是一对、B 与 C 是一对，则 A、B、C 是一串。一次仍只判定一对。裁「类等价」或「生命周期」之后，被并掉的类所牵着的类，改问留下的类——即使当初没有列出过这一对。裁「部分重叠」之后，原来跟这两类牵着的对按现在的类再问。裁「同形异义」或「跳过」只拿掉这一对。上一对的结论不代替下一对的三问。没被任何对牵上的类不进串。人不指定两个对象来比（本期未做），也不靠扫表里的行来补漏。

同一性标准是第二问的前提，不是类等价的前提。没设同一性标准，算不了交集率，仍可按第一问、第三问定案。

**第一问：是不是同一种事物。** 没有单独的机器判据。交集命中大于零是硬线索：对得上号的那些个体是同一个体，包含它们的两个类不可能是两种不相干的东西（自增号等无意义的同一性标准除外，命中不作数）。命中为零不能否定同一种事物：可以是同一种里不相交的两批，也可以是一侧还没有行。定案由 Agent 依证据执行。

**第二问：是不是同一批个体。** 证据是交集率。取值是该类在该源上、按同一性标准取出的个体标识，归一化后做集合重合。个体在源里一行一个或一列一个，不改变怎么取标识：一行一个体，取同一性标准对照的那一列；一列一个体，取列所标识的那个个体。本期只落地行存。没设同一性标准，或有一侧取不出任何取值，这一问沉默。

| 交集率 | 第二问的答案 |
|---|---|
| 有一侧取不出取值 | 沉默 |
| 两边都有取值、接近零 | 现在不是同一批个体 |
| 接近全交 | 现在是同一批个体 |
| 介于中间 | 同一批里只有一部分在两边都有 |

要比的是两个值相不相等，不是把标识存下来。识别取值整列只读取出（超过 5 万行拒绝，先收窄范围再算），归一化后在内存里求交集，值集合算完即弃；落库的只有计数与比率。敏感列的脱敏比对、明文不出库时的指纹比对，本期未做。

**第三问：是不是同一个体的不同时期。** 没有硬证据。状态、日期属性只是线索。定案由 Agent 依证据执行。

**三问合成结论。** Agent 按证据对照下表落稿，人在画布上审结果、可指令推翻。机器建议可以猜，不能代替这张表。

| 第一问 同一种事物 | 第二问 同一批个体 | 第三问 不同时期 | 结论 |
|---|---|---|---|
| 否 | （任意） | （任意） | 同形异义 |
| 是 | 命中大于零 | 是 | 生命周期 |
| 是 | 命中大于零，又不是全交 | 否 | 同一概念。落地：并成一个类挂全部来源（来源覆盖面差异是数据事实） |
| 是 | 全交，或沉默，或命中为零 | 否 | 类等价 |
| （这次不定） |  |  | 跳过 |

两条硬约束：命中大于零时，数据不支持同形异义。命中为零时，数据不支持部分重叠（部分重叠的定义要求个体有交集）。人仍可否决数据，那是不顾证据。

采购「在途设备」和设备「在役设备」：第一问是同一种事物（设备）；第二问交集约三分之一，同一批里只有一部分现在两边都有；第三问定为不同时期。结论是生命周期。两个都叫「账户」、一个资金一个登录：第一问不是同一种事物，结论是同形异义，不必等交集率。资产表还没有行、设备表有行：第二问沉默，不能据此否定类等价。

子类型本期不定。验收问题集用来核本体，不参与判定。

```mermaid
flowchart TD
    S["已上画布、有来源的类"] --> R["结构比对：可能有关的对"]
    S --> E0["入围：不要求不同库、不要求结构相似"]
    R --> E0
    E0 --> STR["对牵成串；一次仍裁一对"]
    STR --> Q1["第一问 同一种事物：Agent 依证据"]
    STR --> Q2["第二问 同一批个体：交集率"]
    STR --> Q3["第三问 不同时期：Agent 依证据"]
    Q1 --> D["合成一项对齐结论"]
    Q2 --> D
    Q3 --> D
    D --> STR
    ...
    D --> H["同形异义：两类留下，结论与证据住工作记录"]
    D -.->|本期不做| I["子类型：只声明继承，不抽公共属性"]
```

图 4：Agent 依证据裁第 2 节的类间关系与生命周期，落可回滚草稿，人在画布上审。类等价：两个类描述同一种事物，写成一个类，两边的表都写成源映射。部分重叠：并入同一个多源类（一个对象挂全部来源）。生命周期：收成一个类，阶段用派生属性读时现算，转化写成关系，由动作写入。同形异义：两个类都留下，配置不动，结论与证据住裁决工作记录（`adj_decision`），不建个体边，源映射也不共用。子类型：一类完全含于另一类，只声明继承、不抽公共属性，本期不做。

上面那一对，定为生命周期之后，配置里只留一个设备类。同一份已发布配置里还有部门类，以及设备到部门的关系；它们来自另一次对齐。
...
节点是对象类型，字段是属性，边是关系。由来虚线和同形异义芯片从裁决工作记录（`adj_decision`）投影，不在配置里。候选对和交集率只作判定的证据，不进配置。类等价、生命周期写入存在结构；部分重叠落成带领域谓词的 match 链（进 `link_types`）；同形异义不进配置，结论与证据住裁决工作记录。

**关系**随**类**的建立和对齐的判定写入配置。新**类**加入后，它与已有**类**之间的**关系**仍走上述过程。

这两节做完之后还要写进配置的，见下一小节。

### 3.3 类上的可执行规定

经过上面两节，配置中已有**类**、**同一性标准**和**源映射**。对齐判定问的是这一对候选类。可以判定为**类等价**、**部分重叠**、**生命周期**或**同形异义**。也可以跳过这次。这些不是类与类关系的穷尽。**子类型**是类与类的一项，本期不定案。

对齐判定这两个**类**之间的**关系**为**生命周期**时，它们描述的是同一个体的两个阶段。配置里因此只写一个**类**。两个源上的表都挂在这个**类**的**源映射**上。两个阶段不再各自作为**类**，而成为这个**类**上阶段**派生属性**的取值。阶段之间的转化写成**关系**，并配一条转化**动作**。
...
引擎加载的是已经发布的那一版配置，例如 3.2 节末尾的 `equipment`。查询和动作都针对这份配置，不针对某张源表。未发布的草稿引擎不加载。语言模型只在四处出现：起草本体配置、给唯一键建议（见 ADR 0010）、给候选对建议、把自然语言编成查询。执行 `derived`、写回时都不在场。模型给建议，不定案，不算交集率。人不发话，模型不自己再生成、不自己写库。请求里的类名、属性名、关系名、动作名对不上已发布配置，引擎拒绝。
...
`equipment`、`in_transit`、`convert` 只写在配置里。引擎的求值源码里没有这些词，也没有按行业分的支路；唯一的例外是无 Key 时的演示实现编译器，它内置演示剧本的几问几答、只服务未配 Key 的 test 演示空间（演示数据按空间名归属），是演示支架，不在正式链路里。换领域等于换一份已发布文本。
...
**写的边界。** 不做跨库分布式事务，失败靠重发与人工修。动作只能写写回里列出的源表，不封装任意存储过程与审批流。经本体的写如何保持语义一致，在学界仍是开放问题：基于本体的数据访问（ontology-based data access, OBDA）通常只覆盖读——本体加映射、查询改写、数据不动，与本系统的读路径同构。本系统对写的回答是工程性的：把写约束成已发布动作的投影，语义的正确性由人审与公理校验承担，不由逻辑保证。

**判定靠证据。** schema 比对与候选对召回出自模型，准确率有限。交集率要求两端有可比的唯一键；识别列直接取出比对，敏感列的脱敏比对本期未做。类与类关系里，子类型本期不定案。合并判错类型比不合并更糟，所以判定与证据全程留痕、发布可回滚。
...
| 本体对齐 | 判定两个类是什么关系（不同库，或同一库的两张表）；对齐时的判定不是类与类关系的穷尽 |
| 结论对照 | 判定只答「是不是同一种事物、同一批个体、不同时期」；落地（并成一个类、建链、时期派生）随证据定。子类型本期不定案。旧称同一 / 阶段 / 仅名称相似。裁决留痕存英文键：same / overlap / stage / name_similar / skip |
| 候选对 | 一串里正在被问的两个类（不同库，或同一库的两张表）；结构比对列出可能有关的对，对牵成串 |
| 交集率 | 候选对两端唯一键取值集合的交集占比；回答「是不是同一批个体」，不回答「是不是同一种事物」；可沉默 |
| 上位对象（已废） | 部分重叠曾经落地时为公共部分另立的类。ADR 0013 起同一概念的表并成一个多源类（一个对象挂全部来源），不再另立 |
| 转化关系 | 记录同一个体从一个阶段转到另一个阶段的关系 |
| 定案 | Agent 依证据落成的结论；结论与证据是裁决留痕（`adj_decision`），不进本体 |
| 画布 | 配置的图形界面：节点是对象类型，字段是属性，边是关系。由来虚线与同形异义芯片从裁决工作记录（`adj_decision`）投影，不进配置 |
...
裁决的结论与证据不进配置：住平台元库的裁决工作记录（`adj_decision`），一行一次裁决，连同当时的证据快照留痕、可回滚。判定落地的形状：类等价并成一个类、挂多个源；部分重叠：同一概念的表并入同一个多源类；生命周期收成一个类，阶段是派生属性；同形异义两类都留下，不建个体边、源映射也不共用。画布上的由来虚线与同形异义芯片从这份记录投影。
...
| 根 | 语法键：`object_types`、`link_types`、`outlets` | 否 |
