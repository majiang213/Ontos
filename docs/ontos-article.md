...
本系统把上述规定写成一份可发布的模型，并用它访问已有系统。多个源上的对象先判别是类等价、部分重叠、生命周期还是同形异义，再进入同一份本体。查询按类型与关系读原库。写入先说明存在上发生了什么（阶段变化、新生一个关联对象等），再写回相应的源。换领域只改这份模型。
...
**类与类。** 两个类还可以是类等价、部分重叠、子类型、同形异义、部分与整体、不相交。这不是封闭清单。对齐候选对时只问其中若干项。类等价是两个类描述同一种现实事物，因而当作一个类；不是因为名字相同。子类型是一类完全落在另一类里面：它的每个个体同时是上位类的个体，并继承上位类的全部属性——上位类就是这条关系的另一端，不是另一种关系。部分重叠是两类的个体有交集、又各有一部分不属于对方。判定看个体在同一性标准下的重合，不看属性是否同名。处理它不用子类型：公共部分抽出来另立一个上位对象。同一性标准复制到上位对象，不从原类移走。两边同名、且不是派生、不是同一性标准所指的属性移到上位对象；没有这样的同名属性时，上位对象只带同一性标准。各自特有的留在原类。上位对象不带继承语义，属性是移上去的，不是继承来的。同形异义是两个类不是在描述同一种东西：两个系统里都有「账户」，一个指资金账户，一个指登录账户，就不能并成一类。部分与整体是组成关系：发动机是车的一部分。它和子类型常被混淆，判别方法很简单：子类型的每个个体同时是上位类的个体，而一台发动机不是一台车，所以它是部分，不是子类型。
...
现有系统要用上本体，第一步是从系统逆向建模：读它的表结构和数据，认出类、属性和关系。系统一多，多一层问题：每个系统只存现实的一个侧面，各起各的名字，各建各的表——这个系统的某个类和那个系统的某个类是什么关系，必须先判定；同一库里的两张表也可能指的是同一种东西。这一步就是本体对齐。对齐时对候选对的判定是封闭的几项，不是类与类关系的穷尽。定义是各项是什么；判定办法是对这一对候选类问三问，用各自的证据，再合成一种结论。
...
同一性标准是第二问的前提，不是类等价的前提。没设同一性标准，算不了交集率，人仍可按第一问、第三问定案。

**第一问：是不是同一种事物。** 没有单独的机器判据。交集命中大于零是硬线索：对得上号的那些个体是同一个体，包含它们的两个类不可能是两种不相干的东西（自增号等无意义的同一性标准除外，命中不作数）。命中为零不能否定同一种事物：可以是同一种里不相交的两批，也可以是一侧还没有行。定案是人。
...
**第三问：是不是同一个体的不同时期。** 没有硬证据。状态、日期属性只是线索。定案是人。

**三问合成结论。** 人按下表定案。机器建议可以猜，不能代替这张表。

| 第一问 同一种事物 | 第二问 同一批个体 | 第三问 不同时期 | 结论 |
|---|---|---|---|
| 否 | （任意） | （任意） | 同形异义 |
| 是 | 命中大于零 | 是 | 生命周期 |
| 是 | 命中大于零，又不是全交 | 否 | 类等价或部分重叠：并成一个类，或保留两个类并立公共对象。人选落地 |
| 是 | 全交，或沉默，或命中为零 | 否 | 类等价 |
| （这次不定） |  |  | 跳过 |

两条硬约束：命中大于零时，数据不支持同形异义。命中为零时，数据不支持部分重叠（部分重叠的定义要求个体有交集）。人仍可否决数据，那是不顾证据。

采购「在途设备」和设备「在役设备」：第一问是同一种事物（设备）；第二问交集约三分之一，同一批里只有一部分现在两边都有；第三问人定为不同时期。结论是生命周期。两个都叫「账户」、一个资金一个登录：第一问不是同一种事物，结论是同形异义，不必等交集率。资产表还没有行、设备表有行：第二问沉默，不能据此否定类等价。
...
flowchart TD
    ...
    D --> H["同形异义：两类留下，写入 class_conclusions"]
    D -.->|本期不做| I["子类型：只声明继承，不抽公共属性"]
```

图 4：人裁的是第 2 节的类间关系与生命周期，定案后改本体。类等价：两个类描述同一种事物，写成一个类，两边的表都写成源映射。部分重叠：另立上位对象。同名公共属性移上去、不带继承语义；没有同名则上位对象只带同一性标准。特有属性留在原类。结论写入 `class_conclusions`。生命周期：收成一个类，阶段用派生属性读时现算，转化写成关系，由动作写入。同形异义：两个类都留下，写入 `class_conclusions`，不建个体边，源映射也不共用。子类型：一类完全含于另一类，只声明继承、不抽公共属性，本期不做。

上面那一对，人选生命周期之后，配置里只留一个设备类。同一份已发布配置里还有部门类，以及设备到部门的关系；它们来自另一次对齐。
...
节点是对象类型，字段是属性，边是关系。由来虚线和同形异义芯片来自 `class_conclusions`，不是 `link_types`。候选对和交集率只用来让人定案，不进配置。类等价、生命周期写入存在结构；部分重叠、同形异义写入根上 `class_conclusions`。
...
引擎加载的是已经发布的那一版配置，例如 3.2 节末尾的 `equipment`。查询和动作都针对这份配置，不针对某张源表。未发布的草稿引擎不加载。语言模型只在四处出现：起草本体配置（含唯一键建议，见 ADR 0010）、给候选对建议、把自然语言编成查询。执行 `derived`、写回时都不在场。模型给建议，不定案，不算交集率。人不发话，模型不自己再生成、不自己写库。请求里的类名、属性名、关系名、动作名对不上已发布配置，引擎拒绝。
...
`equipment`、`in_transit`、`convert` 只写在配置里。引擎的求值源码里没有这些词，也没有按行业分的支路；唯一的例外是无 Key 时的演示实现编译器，它内置演示剧本的几问几答、只服务 test 演示空间（演示数据按空间名归属，与配不配 Key 无关），是演示支架，不在正式链路里。换领域等于换一份已发布文本。
...
**判定靠人。** schema 比对与候选对建议出自模型，准确率有限。交集率要求两端有可比的唯一键；识别列直接取出比对，敏感列的脱敏比对本期未做。类与类关系里，子类型本期不定案。对齐时没有子类型按钮，本期用跳过。合并判错类型比不合并更糟，所以裁决留痕、发布可回滚。
...
| 结论对照 | 类等价 / 部分重叠 / 生命周期 / 同形异义；子类型本期不定案。旧称同一 / 阶段 / 仅名称相似。裁决留痕存英文键：same / overlap / stage / name_similar / skip |
| 候选对 | 一串里正在被问的两个类（不同库，或同一库的两张表）；结构比对列出可能有关的对，对牵成串 |
| 交集率 | 候选对两端唯一键取值集合的交集占比；回答「是不是同一批个体」，不回答「是不是同一种事物」；可沉默 |
| 上位对象 | 部分重叠时抽公共部分立起的类；不带继承语义。同名公共属性移上去；没有同名则只带同一性标准 |
| 转化关系 | 记录同一个体从一个阶段转到另一个阶段的关系 |
| 定案 | 人工裁决的结论，写进本体 |
| 画布 | 配置的图形界面：节点是对象类型，字段是属性，边是关系。由来虚线与同形异义芯片来自 `class_conclusions`，不是 `link_types` |
...
文本两棵树：`object_types` 下每个键是一个**类**，`link_types` 下每个键是一条**关系**。根上还可有 `outlets`、`class_conclusions`。具名的一律用映射，键就是机器名。列表只用于：`when` 规则、效应操作、告知条目、`match` 的配对、`generate` 的项、`values` 的可取值、告知的 `to`、`class_conclusions`（一行一次对齐裁决，没有单独的机器名）。
...
**类与类结论** `class_conclusions`

还活着的类与类结论。一行一次裁决。不是个体边。`kind` 为 `overlap`（部分重叠，必写 `shared` 上位对象名）、`homonym`（同形异义）、`subtype`（子类型，本期引擎不跑）。`classes` 至少两个类名。类等价、生命周期不进这里（已并进存在结构）。部分与整体、不相交本期不定案。同形异义一行一次，不把多对收成一团，不传递。

| 键 | 含义 |
|---|---|
| `kind` | `overlap` / `homonym` / `subtype` |
| `classes` | 参与的类名，至少两个 |
| `shared` | 仅 `overlap`：上位对象名，且必须在 `object_types` 里 |
...
| `class_conclusions` | 根 | 还活着的类与类结论。一行一次裁决 |
| `overlap` `homonym` `subtype` | `class_conclusions.kind` | 部分重叠 / 同形异义 / 子类型（本期引擎不跑 subtype） |
| `shared` | `class_conclusions` | 仅部分重叠：上位对象名 |
...
| 根 | 语法键：`object_types`、`link_types`、`class_conclusions`、`outlets` | 否 |
| `class_conclusions` 列表项内 | 语法键：`kind`、`classes`、`shared` | 否 |
