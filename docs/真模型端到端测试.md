...
`seedDemo`（`src/server/engine/fixture.ts`）给内存库调用 `setComments`。`SqliteFixtureDriver.introspect` 把这份注释挂到列上，表结构抽屉显示「设备序列号」，`proposeObjects` 的 prompt 把 `comment` 写进字段说明。
...
| 生成 | 勾表 → 「生成对象」 | `POST /api/generate_objects` → `getSlot().proposeObjects` → `import_objects`；toast「已生成对象：…」；抽屉收起 |
...
- vitest 对 `POST /api/decide` / `POST /api/generate_objects` / `POST /api/publish` 代人走完整剧本（上一版 API 代人路径，降级，不进本期实现）。
- Playwright / 浏览器自动点。
...
4. **落地前硬闸**（`POST /api/generate_objects`，`import_objects` 之前）：对槽位产出的每个键，若已在 `occupied` 或本次已收下的键里，改名为该类第一条源的 `{connection}_{table}`；仍撞则再加 `_2`。改名只改记录的键，不改编类体。然后才 `import_objects`。返回的 `created` 是改名后的键。
...
- `src/server/engine/llmSlot.ts` — 三槽位；`nlToQuery` 今日只传属性名；`proposeObjects` 今日不接收已有类名；罐头只在同一次 `out` 里加前缀
...
- `src/app/api/generate_objects/route.ts` — 今日 `proposeObjects` 后直接 `import_objects`，不传已有类名
...
- **影响文件：**
  ...
  - `src/server/engine/llmSlot.ts` — `nlToQuery` 的 classes 带 `type` / `values`；`proposeObjects(tables, occupied?)`：prompt 列出已占用类名，规定撞名用 `{connection}_{table}`；罐头认 `occupied`；`disambiguateClassNames` 落地前硬闸。失败写入 `os.tmpdir()/ontos-llm-fail`，脱敏 key。**不**设 `temperature`
  - `src/app/api/generate_objects/route.ts` — 读草稿已有类名传入 `occupied`；槽位返回后、`import_objects` 前跑硬闸。MCP `propose_objects` 不落地，不在 `import_objects` 里静默改名
