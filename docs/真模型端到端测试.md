...
- 本期测试 PR **不要求**生产 `AiSdkSlot` 设 `temperature: 0`（可选后续 PR，见 §6）。
...
| `temperature` | **本期测试 PR 不改生产三次 `this.gen`。** live 与生产的采样口径以生产为准。`temperature: 0` 是可选后续 PR（附兼容模型说明），不和「能不能连上真模型」绑在同一个测试 PR。`callSlot` 若看见 400 且正文提 `temperature`，按上款失败 |
...
`npm test` 与是否导出了 `OPENAI_*` 无关，始终离线。
...
不在测试 PR 里给生产三次 `generateText` 加 `temperature: 0`。不在未开门闩的生产路径上加 30s `abortSignal`。
...
2. **PR2a** 合入后，有密钥的人可以跑槽位级，验证判定器与门闩。含 `nlToQuery` prompt 补枚举、门闩下的 abort/落盘。
3. **PR2b**（可选、可后做）：生产 `temperature: 0`，附兼容模型说明。不挡 PR3。
4. **PR3** 合入后，完整剧本可跑。允许因模型质量红；那是产品信号。设备类 `replaceBlockers` 在 `same` 前非空则失败，不继续。
5. **PR4** 把运行手册写进 README，本设计进 `docs/`。
6. **CI。** 默认流水线只 `npm test`。是否另加带密钥的 nightly / manual job，见 Open Questions。
7. **回滚。** 删 `test:live` 与 `src/tests/live/` 即去掉打网路径；PR1 的剥 key 应保留。PR2b 的 `temperature: 0` 若某接入点不兼容，单独回滚，不影响离线 suite 与 live 门闩。
...
| 中 | 接入点拒绝 `temperature` | 本期生产不设该参数。若 400 正文含该词：落盘、失败、不重试 |
...
1. **是否把 `test:live` 接进 CI。** 需要密钥、配额、以及「模型变差导致红」的值班策略。本设计默认 **不进 CI**。要进的话另开 manual / nightly，仍不并入 `npm test`。
2. **运行者用哪家兼容端点、哪个 `OPENAI_MODEL`。** 生产已经把选择权放在环境变量上。本文不锁模型名，运行手册留空给运行者填。
3. **生产槽位要不要设 `temperature: 0`。** 三个槽位是编译器，设 0 说得通，但兼容端点（尤其 reasoning SKU）可能 400。放到可选 PR2b，附兼容说明。不和 live 门闩绑死。

不列为开放问题的（设计已拍板）：是否剥 key、是否要求 `OPENAI_BASE_URL` 才跑 live、是否复用 questions 跑批、是否改 `load.ts`、是否对真模型 `toEqual` 整份 JSON、是否用 Playwright、是否在未开门闩的生产路径上 30s abort、是否在 `same` 之后再剥 `status`、是否从列表投影推断序列号、是否接受中文过滤值。
...
7. **`callSlot` 对结构化失败与 abort/timeout 重试槽位方法 1 次。** 可重试：`ZodError`、`NoObjectGeneratedError` / `AI_NoObjectGeneratedError`、`TimeoutError`、`AbortError`。不重试 401/403、缺模型名、400+`temperature`。生产槽位仍一次用户可见调用，不是 ReAct。30s abort 与失败落盘只在 `ONTOS_LIVE_LLM=1` 时进入 `AiSdkSlot`。生产 `temperature: 0` 不进测试 PR。
...
四个可独立合并的 PR（PR2 拆成 2a / 可选 2b）。PR1 无 live 用例也能合，且应先合。**PR3 不得在 §5.1–5.2 的关卡顺序写清之前落地；剧本在 `same` 前 `replaceBlockers` 非空必须失败。**
...
### PR2a — live 门闩 + 槽位级 + 判定器（不含生产 temperature）
...
### PR2b — （可选）生产 `temperature: 0`

- **标题：** feat: set temperature 0 on AiSdkSlot compilers
- **依赖：** 无强依赖；建议 PR2a 之后，不挡 PR3
- **影响文件：** `src/server/engine/llmSlot.ts`；README 兼容模型说明
- **简述：** 三个槽位是编译器。若接入点 400 且正文含 `temperature`，调用方应失败而不是静默摘参数。与「能不能连上真模型」解耦。

### PR3 — 完整剧本

- **标题：** test: live playbook empty ontology to convert SN-40217
- **依赖：** PR2a（判定器、`callSlot`、gate、独立 live vitest 配置、prompt 枚举）
- **影响文件：**
...
- **依赖：** PR2a（脚本名已存在）；PR3 合入后补完整剧本小节更完整，但 README 命令在 PR2a 后即可写
- **影响文件：**
  - `README.md` — 「运行」节增加 live 小节：导出三件套 + `ONTOS_LIVE_LLM=1` + `npm run test:live`；写明 `npm test` 始终离线；写明测试会剥 `ONTOS_TOKEN`
