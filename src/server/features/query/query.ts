...
async function queryInner(env: EngineEnv, config: OntologyConfig, driver: SourceDriver, req: QueryRequest): Promise<QueryResult> {
  ...
  const ctx: EvalContext = { identity: req.identity, clock: env.clock, uuid: env.uuid };
  ...
  for (const item of req.expand ?? []) {
    for (const ind of individuals) {
      ...
      const [name, rows] = await expandItem(evalEnv, cls, ind, item, ctx, path);
      bag[name] = rows;
  ...
  if (req.aggregate && aggPlan) {
    ...
    rows = [];
    for (const ind of individuals) {
      const row = await project(cls, ind, req.properties, evalEnv, ctx);
      const bag = expanded.get(ind.key);
