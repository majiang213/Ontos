...
export async function resolveOperand(v: unknown, ctx: EvalContext): Promise<unknown> {
  if (Array.isArray(v)) {
    try {
      return v.map((x) => resolveLiteral(x, ctx)); // in 的值是数组，元素级解析（ISO 串落成秒）
    } catch (e) {
  ...
  if (v !== null && typeof v === "object") {
    ...
    if (typeof rec.property === "string") {
      if (rec.from === "request") {
        try {
          return resolveLiteral(ctx.request?.[rec.property], ctx); // 请求参数：严格，非法拒绝
        } catch (e) {
...
function dataLiteral(v: unknown, ctx?: EvalContext): unknown {
  try {
...
export async function conditionHolds(actual: unknown, condVal: unknown, ctx: EvalContext, dateLike = false): Promise<boolean> {
  const a = dataLiteral(actual, ctx); // ISO 日期串落成秒；脏数据不抛错
