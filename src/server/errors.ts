...
export const MSG = {
  ...
  noClassStages: (name: string) => `${name} 上没有阶段（没有转化关系）`,
  stageNameEmpty: "时期标识不能空",
  stageDupNames: "时期标识不能重复",
  stageWhenDup: "两条规则的条件重复",
  stageCountMismatch: (n: number) => `阶段条数对不上（现在 ${n} 条）`,
  ...
  linkNameMachine: (from: string, to: string) => `关系名不许机器拼接（${from}_to_${to} 这类）——起一个回答领域问题的名字（如 on_equipment/covers），起不出就别建这条链`,
