...
import { replaceBlockers } from "../server/engine/draft/ops/replaceObject";
...
describe("整份替换（replace_object）与 base_rev", () => {
  it("未锁定类可整份替换：关系与摆位保留；无源的残缺生成（有字段）也能换", async () => {
    ...
    await s.editDraft({ op: "create_object", name: "vendor", kind: "thing", description: "旧描述" }, WS);
    await s.editDraft({ op: "add_property", object: "vendor", name: "vendor_no", type: "string" }, WS);
    await s.editDraft({ op: "create_link", name: "vendor_sells", from: "vendor", to: "equipment", match: { from: "vendor_no", to: "serial_no" } }, WS);
    ...
    await s.editDraft({ op: "replace_object", name: "vendor", def }, WS);
  ...
  it("类不存在：replace_object 拒绝并指向 import_objects", async () => {
    const s = await freshStore(tmp);
    await expect(s.editDraft({ op: "replace_object", name: "ghost", def: { kind: "thing", properties: {} } }, WS)).rejects.toThrow("新建请用 import_objects");
  });
  ...
  it("锁定规则：已发布 / 派生 / 多源 / 有源未对照字段 各拒一次；动作与公理走 replaceBlockers 直测；空 map 不误锁", async () => {
    ...
    // 已发布过的类
    await expect(
      s.editDraft({ op: "replace_object", name: "equipment", def: { kind: "thing", properties: {} } }, WS)
    ...
    await expect(
      s.editDraft({ op: "replace_object", name: "with_derived", def: { kind: "thing", properties: {} } }, WS)
    ...
    await expect(
      s.editDraft({ op: "replace_object", name: "multi_src", def: { kind: "thing", properties: {} } }, WS)
    ...
    await expect(
      s.editDraft({ op: "replace_object", name: "unmapped", def: { kind: "thing", properties: {} } }, WS)
  ...
  it("替换后 match 断了：validateSemantics 整步回退，关系与原类体都在", async () => {
    ...
    await s.editDraft({ op: "create_link", name: "po_x_eq", from: "po_x", to: "equipment", match: { from: "sn", to: "serial_no" } }, WS);
    // 替换掉 sn 属性 → match 指向不存在的属性 → 整步回退
    await expect(
      s.editDraft({ op: "replace_object", name: "po_x", def: { kind: "thing", properties: { other: { type: "string" } } } }, WS)
  ...
  it("base_rev：相等才写入； stale 拒绝且不落地；并发下第二个写入拿到 DraftReject", async () => {
    ...
    await s.editDraft({ op: "create_object", name: "vendor", kind: "thing" }, WS, { base_rev: r0 });
    expect(s.getRev(WS)).toBe(r0 + 1);
    await expect(s.editDraft({ op: "add_property", object: "vendor", name: "v1", type: "string" }, WS, { base_rev: r0 })).rejects.toThrow(/草稿已变/);
    ...
    const p1 = s.editDraft({ op: "add_property", object: "vendor", name: "v2", type: "string" }, WS);
    const p2 = s.editDraft({ op: "add_property", object: "vendor", name: "v3", type: "string" }, WS, { base_rev: cur });
    ...
    await s.editDraft({ op: "add_property", object: "vendor", name: "v3", type: "string" }, WS, { base_rev: s.getRev(WS) });
