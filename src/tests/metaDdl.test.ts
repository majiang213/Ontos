...
describe("元库 DDL（三方言）", () => {
  ...
  it("MysqlDatasource 建池开了 multipleStatements；PgDatasource 挂了空闲连接 error（八个 CREATE TABLE 一次下发 / 掉线不炸进程）", () => {
  ...
  it("DDL 设计面：MySQL 每表 ENGINE/CHARSET + 表级 COMMENT；三方言外键与扫描索引齐备", () => {
    expect(MYSQL_DDL.match(/\) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4/g)?.length).toBe(8); // 八表各自声明（中文字段安全；带右括号只数表定义，头注不计）
    ...
    expect(PG_DDL.match(/REFERENCES onto_workspace\(id\)/g)?.length).toBe(7);
    ...
    // PG 注释载体是 COMMENT ON：八表各有表级注释，列注释投影同一份文本（与 MySQL 的 COMMENT 子句对齐维护）
    expect(PG_DDL.match(/COMMENT ON TABLE \w+ IS '/g)?.length).toBe(8);
