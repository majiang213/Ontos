...
/** 采样点列是否全在障碍区外（端点桩区免检：端点本就在自家节点的外扩区里）。 */
function clearOf(samples: Pt[], inflated: RouteRect[], a: Pt, b: Pt): boolean {
  const nearEnd = (p: Pt) => Math.hypot(p.x - a.x, p.y - a.y) < STUB + 8 || Math.hypot(p.x - b.x, p.y - b.y) < STUB + 8;
...
/** 过途经点的曲线：两段 Hermite（途经点处切向取总跨向），端点切向沿法线。途经点所在矩形剔出障碍（用户手拖的位置必须能到）。 */
function throughCurve(from: RouteEnd, waypoint: Pt, to: RouteEnd, obstacles: RouteRect[]): { d: string; mid: Pt; dir: Pt } | null {
  ...
  const t0 = { x: NORMAL[from.side].x * dA * 0.5, y: NORMAL[from.side].y * dA * 0.5 };
  const t2 = { x: NORMAL[to.side].x * dB * 0.5, y: NORMAL[to.side].y * dB * 0.5 };
...
function flowTier(pts: Pt[]): SmoothSeg {
  const n = pts.length;
...
/** 圆角折线档：每个拐角换成小半径 Q（鼓包 ≤14px，比流动档贴线）。 */
function roundedTier(pts: Pt[]): SmoothSeg {
  const n = pts.length;
  if (n === 2) return { d: `L ${pts[1].x},${pts[1].y}`, samples: pts };
  let d = "";
  ...
  for (let i = 1; i < n; i++) {
    ...
    const r = Math.min(14, legA * 0.45, legB * 0.45);
 ...
 *  弯折捏点 = 曲线必经的途经点：先试过途经点的双段曲线（捏点压在线上），不行再绕障拆腿平滑。
...
export function edgePath(from: RouteEnd, to: RouteEnd, obstacles: RouteRect[], waypoint?: Pt | null): { d: string; mid: Pt; dir: Pt } {
  ...
  if (waypoint) {
    ...
    if (wi > 0 && wi < points.length - 1) {
      const a = smoothPath(points.slice(0, wi + 1), obstacles);
      const b = smoothPath(points.slice(wi), obstacles);
      // 途经点处切向：取拆点前后两个路由点的连线方向
      const ddx = points[wi + 1].x - points[wi - 1].x;
      const ddy = points[wi + 1].y - points[wi - 1].y;
      const dl = Math.hypot(ddx, ddy) || 1;
      return { d: `${a.d} ${b.d.replace(/^M /, "L ")}`, mid: waypoint, dir: { x: ddx / dl, y: ddy / dl } };
    }
  }
