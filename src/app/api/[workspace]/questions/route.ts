...
// 失败分阶段记（编译失败/执行出错/答案不符），原因落 detail。跑批逻辑在 features/acceptance/questions。

import { z } from "zod";
import { engineEnv } from "@/server/runtime";
import { getPublished } from "@/server/features/ontology/current";
import { EXPECTED_HINT, parseExpected, runQuestions } from "@/server/features/acceptance/questions";
import { metaStore } from "@/server/meta/store";
import { MSG } from "@/server/errors";
import { BadRequest, bodyJson, rejectRes, requireWriteAuth, respond, workspaceOf } from "@/app/api/_shared";

export async function GET(req: Request, { params }: { params: Promise<{ workspace: string }> }) {
  return respond(async () => {
    const workspace = await workspaceOf(params);
    return { questions: await metaStore().listQuestions(workspace), version: (await getPublished(engineEnv(), workspace)).version };
  });
}
...
export async function POST(req: Request, { params }: { params: Promise<{ workspace: string }> }) {
  ...
  if (url.searchParams.get("run")) {
    return respond(async () => {
      ...
      const r = await runQuestions(engineEnv(), workspace, { onlyId, target });
      if (r.code !== 200) return rejectRes(r);
      const { results, version } = r.value;
  ...
  return respond(async () => {
    ...
    await metaStore().addQuestion(await workspaceOf(params), question, expected?.trim() || undefined);
    return { ok: true };
  });
}
...
export async function DELETE(req: Request, { params }: { params: Promise<{ workspace: string }> }) {
  ...
  return respond(async () => {
    ...
    await metaStore().removeQuestion(await workspaceOf(params), id);
    return { ok: true };
  });
}
