...
export async function POST(req: Request) {
  ...
  try {
    const body = (await req.json()) as { method?: string; params?: { name?: string; arguments?: Record<string, unknown> } };
    ...
    if (name === "query") {
      ...
      try {
        ...
        metaStore().logQuery({ version: getPublished().version, query_json: JSON.stringify(query), row_count: rows.length, ok: true, duration_ms: Date.now() - started });
        return NextResponse.json({ content: { rows, path } });
      } catch (e) {
        metaStore().logQuery({ version: getPublished().version, query_json: JSON.stringify(query), ok: false, error: e instanceof Error ? e.message : String(e), duration_ms: Date.now() - started });
        throw e;
      }
    }
    if (name === "run_action") {
    ...
    if (name === "propose_ontology") {
      const tables = (args.tables as { connection: string; table: string }[]) ?? [];
