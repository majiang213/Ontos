...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { rows, path } = await runQuery(config, await getDriverRegistry(ws), query);
    safeLog(async () => metaStore().logQuery(ws, { version: (await getPublished(ws)).version, question, model: getSlot().name, query_json: JSON.stringify(query), row_count: rows.length, ok: true, duration_ms: Date.now() - started }));
    return NextResponse.json({ question, query, rows, path });
  } catch (e) {
    safeLog(async () => metaStore().logQuery(ws, { version: (await getPublished(ws)).version, question, model: getSlot().name, ok: false, error: e instanceof Error ? e.message : String(e), duration_ms: Date.now() - started }));
