...
**让 AI Agent 安全进出企业的每一个老数据库——不搬一行数据，每一步有定义、有证据、可回滚。**

Ontos 是一个通用本体平台，目标是做成企业级 AI 应用的数据底盘。它从企业存量数据库里逆向出机器可读的业务本体（Ontology），架在全部老库之上，充当人与 AI Agent 共用的读写中间层：查询实时下推各源库拼装作答；写入先核对本体的定义、再按映射投影回原表；最难判错的难题——「两个来源的记录是不是同一个东西」，用数据交集率当硬证据、由人定案。

> **当前进度**：完整闭环已在真实代码中通车——MySQL / PostgreSQL / SQLite 三种源库接入、十个 MCP 工具对外开放、358 条测试全绿（`npm test`）。底盘按生产形态建造：服务无状态可水平扩展、平台元库一行配置切换 MySQL / PostgreSQL、并发编辑有乐观锁保护。本文讲清三件事：它今天能做什么、为什么这样造、下一站去哪里。

![Ontos 画布全景](docs/images/hero-canvas.png)
> 待补素材：整块画布的工作照（对象卡片 + 关系连线 + 工具条可见）。

---

## 一、为什么这一层还没有人做出来

一家真实的公司里，同一个客户的数据散落在采购、设备、财务几套互不相认的老库。想让 AI Agent 替人来办这些事，卡在三道坎上：
...
Ontos 反着来：数据和系统都不动，把这个领域的共同理解写成一份明确的模型（本体），盖在老库上面当读写中枢。建好它只要三步：

1. **生成对象**：勾选源库的表结构，模型读列名与注释，建议这里藏着哪些业务对象和关系，直接放上画布随手可改——建议只进草稿，已发布的本体一动不动；
2. **裁决**：疑似重复的对象进入待确认面板（不同库，或同一库的两张表）：先确认唯一键（凭什么认定两条记录指向同一个体），再算交集率（两边标识字段取值的重合比例），你据此把这两类裁决为五种关系之一：同一 / 部分重叠 / 阶段 / 仅名称相似；
...
![从勾表到对象上画布](docs/images/demo-import.gif)
> 待补素材：表结构抽屉勾选三张表 → 点「生成对象」→ 抽屉收起、对象出现在画布上。

第一组疑似重复出现：采购库的「在途设备」和设备库的「在役设备」。算一下序列号交集率——约三分之一相同，你据此把这两类裁决为**阶段**关系（同一批设备的先后时期），两类合为一个对象挂双源，阶段变成读时现算的派生字段，转化动作随之自动建立。第二组设备 × 资产序列号全部重合，把它们裁决为**同一**，资产台账并入同一对象。

![交集率与裁决](docs/images/demo-adjudicate.gif)
> 待补素材：「待确认」面板 → 算交集率 → 证据数字停留两秒 → 定案。

发布后问一句「列出在役设备及其所属部门」——答案是三个源库的实时拼装，附取数路径（这次查询是怎么执行的逐步说明）：

![外部 Agent 问数](docs/images/demo-query.gif)
> 待补素材：同屏分栏，终端 Agent 调 MCP `query` + 网页画布，答案与取数路径同屏可见。

再执行动作「验收入库」，点名某台在途设备：设备源与资产台账各落一行新记录并立起保修卡。**再问一次，这台设备的阶段已经是「在役」。**

![动作写回前后对比](docs/images/demo-writeback.png)
> 待补素材：动作执行结果 + SQLite 客户端里三个源表的新行。
...
- **模型只有三个槽位**：建模草稿建议、重复关系倾向、自然语言编结构化请求。查询的执行不含模型——编译、下推、跨源对齐、拼装全是确定性代码，同一个问题永远走同一条取数路径。
...
一句话总结分工：模型负责理解语言和表结构的语义，引擎负责精确执行，人负责只有人该负责的定案。
...
| 测试永远离线 | 测试套件启动即剥掉模型 Key 与写令牌，358 条用例不连外网、随时可跑 |
...
| 模型接入 | Vercel AI SDK + Zod 结构化校验，OpenAI 兼容协议任意接；无 Key 时离线回退，功能可演示 |
...
```bash
npm install
cp .env.example .env   # 全部变量可选：不配 Key 也能走主要流程（离线回退）
npm run demo:seed      # 把十二套演示库写成 .ontos-demo/ 下的 SQLite 文件
npm run dev            # http://localhost:6688
```

最快的入口：打开页面把左上角工作空间切到 `test`——自带现成本体和四个 fixture 连接，直接体验画布、裁决面板和验收问题集跑批。想要完整的三波剧本（物 → 人 → 客与货，配真模型效果最佳），照 [docs/真模型端到端测试.md](docs/真模型端到端测试.md) 走，全程约三十分钟。
...
外部 Agent（Claude Code 等）经 MCP 端点驱动 Ontos，人的关卡（裁决、发布）刻意不开放给自动流程：
...
**已经通车的**：三种源库连接与脱敏采样 · AI 生成对象草稿 · 候选对发现与交集率计算 · 五种关系裁决（子类型预留）· 版本化发布与历史回滚 · 自然语言问数（含取数路径）· 动作执行与多源写回 · MCP 十工具与配套 skill · 验收问题集批量回归 · 多工作空间隔离 · 无状态多实例部署。
...
3. **子类型裁决落地**：第四种关系的完整流程开放；
4. **建模质量回归**：标注测试库上量化对象识别与映射的查准查全，北星指标 = 人工修改率 + 裁决接受率；
5. **裁决自动化分级**：接受率长期达标的「仅名称相似」类升级为机器预分拣加人工抽检；合并类无论接受率多高都不毕业——一次错并的代价与一百次对的好处不对称。
...
| [docs/真模型端到端测试.md](docs/真模型端到端测试.md) | 三波走查的完整演示剧本 |
...
| 素材文件名 | 拍什么 | 时长建议 |
|---|---|---|
| `docs/images/hero-canvas.png` | 画布全景：若干对象卡 + 连线 + 工具条，深色主题更出片 | 截图 |
| `docs/images/demo-import.gif` | 表结构抽屉勾选三张表 → 「生成对象」→ 抽屉收起、对象上画布 | 20–30 秒 |
| `docs/images/demo-adjudicate.gif` | 「待确认」面板：先确认唯一键 → 点「算一算交集率」→ 证据数字停留两秒 → 定案 | 30–40 秒 |
| `docs/images/demo-query.gif` | 同屏分栏：终端调 MCP `query` 问「在役设备及其所属部门」+ 网页画布，答案与取数路径可见 | 30–40 秒 |
| `docs/images/demo-writeback.png` | 动作「验收入库」返回结果 + SQLite 客户端里三个源表的新行 | 截图拼图 |
| 完整视频 | 以上串联配音讲解，上传后把链接填进「两分钟看懂」一节末尾 | 2–3 分钟 |

录制工具：macOS 用 `⌘⇧5` 或 CleanShot X 录屏，GIF 用 `ffmpeg -i in.mov -r 15 out.gif` 压制（宽度压到 960px 以内控制体积）。每个片段动手前先排练一遍脚本再开录，减少废片。
