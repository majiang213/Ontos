...
![Ontos 画布全景](docs/cover-canvas.png)
...
采购、设备、资产三个库都接好后，第一组疑似重复出现：采购库的「在途设备」和设备库的「在役设备」。算一下序列号交集率——约三分之一相同，你据此把这两类裁决为**生命周期**（同一批设备的先后时期），两类合为一个对象挂双源，阶段变成读时现算的派生字段，转化动作随之自动建立。第二组设备 × 资产序列号全部重合，把它们裁决为**类等价**，资产台账并入同一对象。
...
```mermaid
flowchart LR
    M["模型建议<br/>软证据"] --> O["交集率<br/>硬证据"] --> H["人定案<br/>结论"]
```
...
最快的入口：打开页面把左上角工作空间切到 `test`——自带现成本体和七个 fixture 连接，直接体验画布、裁决面板和验收问题集跑批。想要完整走查（设备一生 + 办公线 · 八套接入系统，配真模型效果最佳），照 [docs/真模型端到端测试.md](docs/真模型端到端测试.md) 走，全程约三十分钟。
