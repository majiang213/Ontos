...
2. **裁决**：疑似重复的对象进入待确认面板（不同库，或同一库的两张表）：先确认唯一键（凭什么认定两条记录指向同一个体），再算交集率（两边标识字段取值的重合比例），你据此把这两类裁决为类等价 / 部分重叠 / 生命周期 / 同形异义（也可跳过这次）；
...
第一组疑似重复出现：采购库的「在途设备」和设备库的「在役设备」。算一下序列号交集率——约三分之一相同，你据此把这两类裁决为**生命周期**（同一批设备的先后时期），两类合为一个对象挂双源，阶段变成读时现算的派生字段，转化动作随之自动建立。第二组设备 × 资产序列号全部重合，把它们裁决为**类等价**，资产台账并入同一对象。
...
**已经通车的**：三种源库连接与脱敏采样 · AI 生成对象草稿 · 候选对发现与交集率计算 · 对齐裁决（类等价 / 部分重叠 / 生命周期 / 同形异义；跳过这次；子类型本期不定案）· 版本化发布与历史回滚 · 自然语言问数（含取数路径）· 动作执行与多源写回 · MCP 十工具与配套 skill · 验收问题集批量回归 · 多工作空间隔离 · 无状态多实例部署。
