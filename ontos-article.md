...
中间这条路是：规定盖在已有系统上。没有映射，就不能投影。个体不为一次动作之外留在引擎里。
...
判定不从「两个库的全部表」铺开。人先在表结构抽屉里勾选表和列，点生成：对象直接上画布，表本身不成节点。没有预览卡，也没有确认草稿卡。机器只在**已上画布的对象**之间找跨源候选对。单源对象不进裁决，可以直接发布。发布之后再加表，只裁新冒出来的对。

前两步自动跑，第三步才打断人。中间至多浮一张卡：先问识别字段（有抉择才问），再问候选对。同一时间底中只有这一张。点节点改字段、改识别字段，随时可以，不必再走一遍确认。
...
第二步，数据交集率。对候选对两端已设的识别字段归一化后算取值交集。没设识别字段，卡上写「两边对不上号」。字段像序列号、身份证时，生成后会先问设不设为识别字段；点节点也能改。

还是在途设备和在役设备：识别字段都是出厂序列号，去掉横杠、统一大小写后再比。采购里 120 台，设备里 100 台，序列号重合 40 台，交集率约三分之一。接近全交才像同一批东西现在都在两边；接近零才像完全两拨人。三分之一落在中间，再看字段——设备侧有状态、启用日，倾向阶段：同一批设备，只是有的还在途、有的已在役。
...
要比的是两个值相不相等，不是把标识存下来。只读采样，内存里算，集合算完即弃。已脱敏的按脱敏值比；明文出不了库的，下推到源里算指纹再比。

第三步，人工裁决。前两步给出的是建议和交集率，定案只能是人。语言模型建议大约六到八成准；交集率把范围收到一两种，选哪一种仍要人拍板。跳过按各自独立处理，以后可以改。子类型本期不定。

在途对在役：建议曾是同一，交集约三分之一，倾向阶段。人定阶段——两个类收成一个「设备」，挂两个源，并增加转化关系。账户对账户：人定仅名称相似，两个类各自独立，互不映射。定案按上一节的表写进本体。验收问题集用来事后核本体，不参与这一步。
...
flowchart TD
    ...
    C --> D["第三步 人工裁决"]
    D --> E["类等价：一个类，多个源映射"]
    D --> F["子类型：上位类 + 特有属性留在原类"]
    D --> G["生命周期：一个类 + 派生属性 + 关系"]
    D --> H["同形异义：两个类独立，无关系"]
```

图 4：人裁的是第 2 节的类间关系与生命周期，定案后改本体。类等价：两个名字并成一个类，两边的表都写成源映射。子类型：公共属性立上位类，特有属性留在原类，两个原类作为它的子类型。生命周期：收成一个类，阶段用派生属性读时现算，转化写成关系，由动作写入。同形异义：两个类都留下，不建关系，源映射也不共用。一类完全含于另一类、只声明继承、不抽公共属性的那种，本期不做。
...
object_types:
  equipment:
    ...
    identity: serial_no            # 属性名，默认配对键。某源没有此列时，该源条目写 key（附录 B）
      ...
      status:
        ...
        derived:                   # 派生：不对应源列。规则里写到的源，引擎才访问
          ...
          - when:
              ...
              device: false        # 设备源必须无行；过滤时只取配对键列，用来排除
            value: in_transit
          - when:
...
经过上面三步，配置中已有**类**、**同一性标准**和**源映射**。第三步判定的是这两个被比对的**类**之间的**关系**，该项为**类等价**、**子类型**、**生命周期**或**同形异义**之一。
...
例如：采购系统有尚未验收入库的设备，设备系统有已经投入使用的设备，逆向建模时各建成一个**类**（在途设备、在役设备）。第三步判定这两个**类**之间的**关系**为**生命周期**之后，配置里只写「设备」这一个**类**；两张表成为它的两条**源映射**；「在途」「在役」是阶段**派生属性**的取值。
...
同一**类**上往往还有别的变化。把一台在役设备调到另一个部门，改变的是归属，不是阶段。把一台设备标为报废，改变的是另一件事。已有的转化**动作**不能用来做这些事。因此调拨、报废要各自写成**动作**。每一条**动作**须写明三件事：变化发生前个体必须满足什么（前置），存在上变成什么（效应），变化写入哪张源表的哪些列（写回）。引擎只执行配置里写明的**动作**。某一变化没有对应的**动作**，引擎就不能把这次变化写入源表。
...
`equipment`、`in_transit`、`convert` 只写在配置里。引擎源码里没有这些词，也没有按行业分的支路。换领域等于换一份已发布文本。
...
| 换领域时 | 更换这份文本 | 不改 |
| 如何检验 | 版本比较；`description` 是否与可求值字段一致 | 源码中不出现领域名词 |
...
**`aggregate` 改变返回的形态。** 不给它，返回一台台设备；给了它，返回分组统计——按部门分组、每组多少台。
...
属性名配字面值，就是等值。`{ "status": "in_service" }` 与 `{ "status": { "eq": "in_service" } }` 相同。
...
自然语言由 Agent 编成上述请求。类名、属性名、关系名不得写死在 Agent 中。编请求之前，Agent 按需读取已发布配置的一个视图，而不是整份文本。`description` 供阅读；填进 JSON 的是 `name`。
...
过滤值换成 `{ "status": "in_transit" }` 时，按同一份规则翻译：命中采购源有行、设备源无行那条。`when` 写了两个源，两个都查。设备源只取 `serial_no`；采购源取本次要的列，条件是 `sn` 不在这批序列号里。采购记录在役之后不会删，排除靠的是设备源没有同一序列号，不是采购源缺行。`po_item` 上没有阶段列。
...
flowchart TD
    ...
    J2 --> D
    D --> O["排序、分页、聚合：能下推的单源算，跨源的在取回的行上算"]
...
调岗做完：一条 `change`，下面两行 `change_line`——`update` 旧任职、`create` 新任职。整包发给出站，不拆成两条通知。
...
**判定靠人。** schema 比对与候选对建议出自模型，准确率有限。交集率要求两端有可比的识别字段；明文出不了库时只能比指纹。五种结论里子类型本期不定案。合并判错类型比不合并更糟，所以裁决留痕、发布可回滚。
...
| 同一性标准 | 判定两条记录指向同一个体所凭的特征。界面上叫识别字段 |
...
| 写入 | 按已发布动作改变存在的请求 |
| 告知 | 写回之后，把这次变更收成一条泛化的变更事件，发给没有源映射的出站 |
| 出站 | 已声明的收件地址，不是源表 |
| 留痕 | 每次动作的记录：哪条动作、哪个体、每条投影的成败；平台留它，不留业务行 |
| 归一化 | 比对前把识别字段洗成统一格式；只在内存，不改源值、不落库 |
...
| `transition` | 转化：同一个体，`property` 从 `from` 阶段到 `to` 阶段 |
...
查询节点：`object`、`identity`、`properties`、`filter`、`order`、`limit`、`aggregate`、`expand`。`expand` 项内再写 `relation`、`properties`、`filter`、`expand`。
