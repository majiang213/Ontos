...
| `propose_action` | 对某个类产一条动作建议（只建议，不写草稿）。返回 `{ name, action }`。**一律传 `space: "draft"`**——缺省读已发布，对草稿类会空转 | `{ object, space: "draft" }` |
| `edit_draft` | 把动作定义写进草稿（op 只用 `set_action` / `remove_action`），必带 `base_rev` | `{ op, ...，base_rev }` |

发现类工具必须传 `space: "draft"`。`edit_draft` 不接受 `space`。

## 方法论（四步）：看现有动作 → 取模板或读回 def → 完善 → 写进草稿请人发布
