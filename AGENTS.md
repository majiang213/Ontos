...
_UI 说法_：裁决按钮「类等价 / 部分重叠 / 生命周期 / 同形异义 / 跳过」（`VERDICT_LABELS`；decisions API 与 adj_decision 留痕存英文键 `same / overlap / stage / name_similar / skip`，汉字只当描述不当 Key）。_Avoid_: 五种结论、三结论 / 二期三结论等任何把结论打包计数的说法（当类与类关系的章节名或穷尽表）——类与类关系不是封闭清单：未定案的逐个点名（子类型、部分与整体、不相交），能落的也逐个点名（类等价、部分重叠、生命周期、同形异义；跳过是动作不是关系）
