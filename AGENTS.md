...
| 疑似重复 | 「待确认」面板②区块（①唯一键确认后才解锁） | `/api/list_candidates` | `listCandidates`（建议原语 `proposePairs`） | 无（关卡在人） |
...
- **步骤动作 = 画布浮动卡**：左上一条工具条（连接/新建对象、待确认、发布与版本、验收问题集；关卡用按钮，其余用文字），待确认面板（底中，唯一键 → 疑似重复两段）、表结构抽屉开关（左下）、对象编辑卡与关系详情（右侧）。选表并入**表结构抽屉**（勾选表 → 生成对象，不取业务行）；表结构和映射都收在底部抽屉。
