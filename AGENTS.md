...
**匹配键（identity）** 🔴：
声明凭哪条属性认定两条记录指向同一个体（身份证/手机号）。交集验证与问数对齐共用。
...
**五关系类型** 🔴：
疑似同义对象合并的五种答案：①同一（两个类描述同一种现实事物 → 合并单对象挂多源）②部分重叠（描述的个体有交集又不是同一批 → 上位对象+各自特有属性）③生命周期阶段（同一个体的不同时期 → 统一对象+状态+转化关系）④子类型（V2 预留）⑤仅名称相似（不是同一种东西，各自独立）。判定看实质，不看名字、不看表是多行还是多列、不看字段是否同名。不同库和同一库的两张表都适用。
...
**候选对** 🔴：
一串里正在被问的两个对象（不同库，或同一库的两张表）。一对的结论会改这一串里还该问谁，不改下一对该判成哪一种。模型提议按草稿内容快照落 `adj_candidates`：同一投喂形状（类名/连接集/字段名）只问一次模型，定案过滤在读时套——计数因此稳定，不当计算器的模型只补倾向。
_UI 说法_：疑似重复（面板②区块标题「疑似重复」，入口按钮「待确认」——它同时装唯一键确认）；「候选对」只留在文档与代码。

**疑似重复串** 🔴：
通过候选对能牵到一起的一组对象。A 与 B 是一对、B 与 C 是一对，则 A、B、C 是一串。一次仍裁一对；裁完只在这一串里再问。没被牵上的对象不进串。
_UI 说法_：不出现。待确认仍一次一对。_Avoid_: 簇、连通分量、独立题、一次裁整串

**交集率** 🔴：
两个源标识字段（归一化后）的集合重合度 = |∩|/max(|A|,|B|)。回答"两边记录是不是同一批现实实体"——命名会骗人，数据不会。
...
**三层证据链** 🔴：
判定按三问：同一种事物（人；命中大于零是硬线索）→ 同一批个体（交集率，可沉默）→ 不同时期（人）。结构建议只列出可能有关的对，不是这三问的答案。候选对属于一串，裁完只在串里再问。命中大于零则数据不支持仅名称相似；命中为零则数据不支持部分重叠。两版建议是同一裁判：看过交集率后的第二版以清单快照里的第一版为锚。验收问题集不参与裁决。

**上位对象** 🔴：
②的产物：抽公共部分立起的类。同名公共属性移上去；没有同名则只带唯一键。各源对象只留特有属性。
