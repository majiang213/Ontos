...
| 选择演示库文件 | 连接表单下拉（也可手动填路径） | `/api/list_sqlite_files` | `listSqliteFiles` | 无 |
...
- **步骤动作 = 画布浮动卡**：左上一条工具条（连接/新建对象、待确认、发布与版本、验收问题集），待确认面板（底中，唯一键 → 疑似重复两段）、表结构抽屉开关（左下）、对象编辑卡与关系详情（右侧）。选表并入**表结构抽屉**（勾选表 → 生成对象，不取业务行）；表结构和映射都收在底部抽屉。
